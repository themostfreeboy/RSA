...
bool UserCheck(char UserName[], char UserPassword[])//检测用户名与密码是否匹配
{
	unsigned char MD5Password[17];//用于存储将输入的密码进行MD5散列运算后的结果
	UserDataInit();//用户名与密码对应信息的初始化
	MD5Calc((unsigned char*)UserPassword, MD5Password);//将输入的密码进行MD5散列运算，将散列运算后的结果存储在MD5Password数组中
	MD5Password[16] = '\0';//数组结尾写入'\0'结束符
	for (int i = 0; i < NUM; i++)//比较输入密码经过MD5散列运算后的值与之前存储的值，与对应的用户名是否匹配
	{
		if (strcmp(UserName, userdata[i].UserName) == 0 && strcmp((char*)MD5Password, userdata[i].UserPassword) == 0)
		{
			return true;
		}
	}
...
void UserPasswordInit(char x[17],char y[33])//信息初始化中密码字符串向十六进制的转化，为了与MD5函数算出的结果存储形式一致
{
