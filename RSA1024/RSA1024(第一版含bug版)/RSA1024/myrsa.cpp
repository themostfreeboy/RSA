...
private_key MakePrime(const unsigned long mode)//生成大质数p,q及n,e
{
	...
	pkey.create(prand[0], prand[1]);//计算生成两个大质数p,q
	return pkey;
}
...
int rsa1024_file_code(const char* readfilename, const char* writefilename, private_key &pkey, const unsigned long mode)//RSA算法1024位密码强度加密，文件头部写入3个byte标志位
{
	...
	vlong m;
	vlong c;
	long read_time=read_filesize/128+1;
	...
	//最后一次的加密处理
	//最后一部分有数据部分的处理
	unsigned long left_byte=read_filesize-(read_time-1)*128;
	int j=0;
	...
	tempchar=128-left_byte;//为了避免填充的数据为0(left_byte数范围0~127)，所以采用(128-left_byte)这个数填充(此数范围1~128)
	for(int i=left_byte%4;i<4;i++)
	{
		tempint=(tempint<<8)|(tempchar&(0xff));
	}
	temp_in[j]=tempint;
...
int rsa1024_file_decode(const char* readfilename, const char* writefilename, const private_key pkey)//RSA算法1024位密码强度解密，校检文件头部写入的3个byte标志位
{
	...
	//最后一部分有数据部分的处理
	unsigned long left_byte=128-temp_out[31]&(0xff);
	int j=0;
	for(j=0;j<left_byte/4;j++)
	{
		tempint=temp_out[j];
